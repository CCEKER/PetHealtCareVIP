...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
		...
		coreAssembly.assemble(container: rootContainer)
		
		let assemblies: [Assembly] = [coreAssembly]
		_ = Assembler(assemblies, container: rootContainer)
		
        applicationFlowCoordinator = ApplicationFlowCoordinator(window: window, container: rootContainer)
